export async function onRequestPost({ request, env }) {
  try {
    ...
    if (!env.EMAILOCTOPUS_API_KEY || !env.EMAILOCTOPUS_LIST_ID) {
      return new Response(JSON.stringify({ error: "Newsletter not configured - missing EMAILOCTOPUS_API_KEY or EMAILOCTOPUS_LIST_ID", debug: { hasKey: !!env.EMAILOCTOPUS_API_KEY, hasList: !!env.EMAILOCTOPUS_LIST_ID, keys: Object.keys(env || {}) } }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
